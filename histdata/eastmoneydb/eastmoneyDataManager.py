...
class eastmoneyDataCenter():
    def __init__(self,dataRoot):
        self.localStorage = localStorage(dataRoot,'eastmoney','D')
        ...
        self.logger = logbook.Logger('eastmoney')
    ...
    def downloadAndStoreKDataByCode(self,code = '',period="D",timeFrom = None,timeTo = None):
        ...
        if(histDataOne is  None):
            ...
            return True
    def __genStrFunc(self,bookName,modelName,functionName,**kwargs):
        '''mid
        原计划通过此函数生成调用参数传递给vba执行
        得知执行此过程时也必须要打开被调用过程所在excel以打开choice终端后，直接在excel中做输入输出更加方便
        '''
        strFunc = bookName + '!' + modelName + '.' + functionName + '('
        
        for key in kwargs:  
            print "another keyword arg: %s: %s" % (key, kwargs[key])
            
            
        strFunc = strFunc + '''"''' + kwargs['dataPath'] + '''"'''
        strFunc = strFunc + ''',"''' + kwargs['symbol'] + '''"'''
        strFunc = strFunc + ''',"''' + kwargs['dateFrom'] + '''"'''
        strFunc = strFunc + ''',"''' + kwargs['dateTo'] + '''"'''
    ...
    def __initVba(self):
        ...
        self.bookName = 'mid_hist.xlsm'    
        self.bookPath = os.path.abspath(os.path.join(os.path.dirname(__file__),self.bookName))        
    ...
    def __downloadHistDataOne(self,histDataType = 'dataWithMA',code = '',period="D",timeFrom = None,timeTo = None):
        ...
        strpara = self.__genStrFunc(self.bookName, modelName, functionName,dataPath = dataPath,symbol = symbol,dateFrom = dateFrom,dateTo = dateTo)
        
        status = self.xlapp.executeexcel4macro(strpara)

        df = pd.read_csv(dataPath+os.sep+('%s.csv'%code),index_col=0,parse_dates=[0],encoding='gbk')  
    ...
    def downloadHistData(self,codeList = None,periodType = 'D',timeFrom = None,timeTo = None,storageType =  'mongodb'):
        ...
        #mid ---------------------------------------------------------------------
        #mid 借用tushare处理
        #mid tushare 的 dataWithMA的index是"2010-01-22"格式
        #mid tushare 的 其他数据index 是 timestamp格式，所以需要转化
        histDataType = "hfq"
        if(storageType == 'mongodb'):
